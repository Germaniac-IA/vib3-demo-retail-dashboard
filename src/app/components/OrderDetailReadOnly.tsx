          ...
          <div style={{ border: "1px solid #eee", borderRadius: "8px", overflow: "hidden" }}>
            {order.items.map((item: any, idx: number) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", borderBottom: idx < order.items.length - 1 ? "1px solid #f0" : "none", fontSize: "13px" }}>
                <span>{item.quantity} × {item.product_name}{item.attribute_value_name ? <span style={{display:'inline-flex',alignItems:'center',gap:'4px',background:'#1a56db',color:'#fff',padding:'2px 10px',borderRadius:'20px',fontSize:'12px',fontWeight:700,marginLeft:'8px'}}>{item.attribute_type_name ? item.attribute_type_name+': ' : ''}{item.attribute_value_name}</span> : null}{item.attribute_allocations ? <span style={{display:'flex',flexDirection:'column',gap:'1px',marginTop:'2px',marginLeft:'20px'}}>{item.attribute_allocations.map(a=>{const label=a.attribute_type_name?a.attribute_type_name+': ':'';return <span key={a.attribute_value_id} style={{fontSize:'12px',color:'#555'}}>{a.quantity}x <b>{label}{a.attribute_value_name||a.attribute_value_id}</b></span>;})}</span> : null}</span>
