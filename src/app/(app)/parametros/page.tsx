...
const TABS = [
  ...
  { key: "atributos_tipos", label: "📏 Tipos de Atributo" },
  { key: "atributos_valores", label: "📐 Valores de Atributo" },
  { key: "entidades", label: "🏢 Entidades / Clubes" },
];
...
export default function ParametrosPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "4px", overflowX: "auto", whiteSpace: "nowrap", paddingBottom: "8px", marginBottom: "12px", scrollbarWidth: "thin" }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            style={{ padding: "6px 14px", borderRadius: "8px", border: "none", background: activeTab === t.key ? "#1a1a2e" : "#f0f0f0", color: activeTab === t.key ? "#fff" : "#666", cursor: "pointer", fontSize: "12px", fontWeight: 700, flexShrink: 0 }}>
            {t.label}
          </button>
        ))}
      </div>
      <div>
        ...
        {activeTab === "estados_pago" && <PaymentStatusesABM />}
        {activeTab === "insumos" && <InputItemsABM />}
        {activeTab === "atributos_tipos" && <AttributeTypesABM />}
        {activeTab === "atributos_valores" && <AttributeValuesABM />}
        {activeTab === "entidades" && <EntitiesABM />}
      </div>
    </div>
  );
}
