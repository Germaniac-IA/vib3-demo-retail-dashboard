...
export default function ProductosPage() {
  ...
  function loadAllInputs() {
    fetchJson<any[]>("/input-items").then(setAllInputs).catch(() => {});
  }
                ...
                <Card key={p.id} style={{ opacity: p.is_active === false ? 0.55 : 1, border: p.is_active === false ? "1px dashed #ccc" : undefined, minHeight: "180px" }}>
                  <div style={{ display: "flex", gap: "12px", cursor: "pointer", minHeight: "130px" }} onClick={() => openEdit(p)}>
                    <div style={{ flex: 1 }}>
                      ...
                      {p.computed_cost > 0 ? <div style={{ fontSize: "11px", color: "#888", marginTop: "2px" }}>Costo: ${Number(p.computed_cost).toLocaleString("es-AR")} (insumos)</div> : p.cost_price > 0 ? <div style={{ fontSize: "11px", color: "#888", marginTop: "2px" }}>Costo: ${Number(p.cost_price).toLocaleString("es-AR")}</div> : null}
