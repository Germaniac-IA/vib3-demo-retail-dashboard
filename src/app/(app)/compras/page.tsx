            ...
            {order.items.map((item: any, idx: number) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", borderBottom: idx < order.items.length - 1 ? "1px solid #f0f0f0" : "none", fontSize: "13px" }}>
                <span>
                  ...
                  {item.attribute_value_name || (item.attribute_allocations && item.attribute_allocations[0] && item.attribute_allocations[0].attribute_value_name) ? <span style={{display:'inline-flex',alignItems:'center',gap:'4px',background:'#1a56db',color:'#fff',padding:'2px 10px',borderRadius:'20px',fontSize:'12px',fontWeight:700,marginLeft:'8px'}}>{(item.attribute_type_name || (item.attribute_allocations && item.attribute_allocations[0] && item.attribute_allocations[0].attribute_type_name)) + ': '}{item.attribute_value_name || (item.attribute_allocations && item.attribute_allocations[0] && item.attribute_allocations[0].attribute_value_name)}</span> : null}
